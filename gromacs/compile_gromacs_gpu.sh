...
cd build
FLAGS="-xCORE-AVX2 -g -static-intel"; CFLAGS=$FLAGS; CXXFLAGS=$FLAGS; CC=mpiicc; CXX=mpiicpc  cmake .. -DCMAKE_INSTALL_PREFIX=$GROMACS_BINARY -DGMX_BUILD_OWN_FFTW=ON -DREGRESSIONTEST_DOWNLOAD=ON -DGMX_MPI=on -DGMX_GPU=CUDA -DGMX_CUDA_TARGET_SM="30;35;52;60;61;70;75"  -DGMX_OPENMP=ON -DGMX_SIMD=AVX2_256 -DGMX_CYCLE_SUBCOUNTERS=ON -DGMX_OPENMP_MAX_THREADS=32 -DBUILD_SHARED_LIBS=OFF  -DGMX_BUILD_HELP=OFF 
make 
make check
make install 